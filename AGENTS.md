...
- **`src/automata/`** is the grammar engine:
  - `primitives.py` — three FSMs (`NumberFSM`, `StringLiteralFSM`, `ExactMatchFSM`). Each implements `allowed_characters() -> set[str]` — this is the contract the trie DFS uses to prune illegal tokens.
  ...
  - `pda.py` — `JSONPushdownAutomaton` enforces JSON structure via a `Scope` stack and `remaining_keys` tracking. Its `allowed_characters()` is the mask the DFS consumes.
...
- **`ExactMatchFSM`** is used for anything with a fixed vocabulary: booleans (`true`/`false`), `null`, enum literals, JSON keys, function names. It filters candidates char-by-char via `allowed_characters`.
