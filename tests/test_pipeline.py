...
@patch("src.engine.pipeline.compile_tools")
def test_pipeline_initialization(mock_compile, mock_model, mock_trie, sample_functions):
    """
    Proves that the pipeline correctly compiles the tools once during init
    and securely separates the router table from the extractor table.
    """
    mock_compile.return_value = {
        "name": "mock_name_fsm", 
        "location": "mock_location_fsm"
    }
    ...
    assert pipeline.extractor_table == {"name": "mock_name_fsm", "location": "mock_location_fsm"}
...
@patch("src.engine.pipeline.compile_tools")
def test_process_prompt_success(mock_compile, mock_generator_class, mock_prompt_builder, 
                                mock_model, mock_trie, sample_functions):
    """
...
@patch("src.engine.pipeline.ConstrainedGenerator")
@patch("src.engine.pipeline.compile_tools")
def test_process_prompt_hallucination_trap(mock_compile, mock_generator_class, 
                                           mock_model, mock_trie, sample_functions):
    """
